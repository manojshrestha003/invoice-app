...
export default function ProfilePage() {
  ...
  const handleEditProfile = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/users/${user._id}`, {
        ...
        body: JSON.stringify({
          ...editForm,
          avatar: user.avatar || "" // Explicitly include avatar from state
        }),
      });
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const imageUrl = updatedUser.secure_url;

      // 2. Update local state immediately for preview (functional update is safer)
      setUser((prev: any) => ({ ...prev, avatar: imageUrl }));
      toast.success("Avatar uploaded! Save profile to persist changes.");
    } catch (err: any) {
          ...
          <div className="bg-[#111111] border border-white/10 rounded-[2rem] p-8 w-full max-w-md shadow-2xl relative overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex flex-col items-center mb-8 relative">
              <div className="relative group">
                <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-purple-500/50 flex items-center justify-center bg-[#161616]">
                  {user.avatar ? (
                    <img src={user.avatar} alt="Avatar Preview" className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-2xl font-black">
                ...
                <button
                  ...
                  className="absolute -bottom-1 -right-1 w-8 h-8 bg-purple-600 hover:bg-purple-500 text-white rounded-full flex items-center justify-center shadow-lg transition-colors border-2 border-[#111111]"
                >
              ...
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-3">Change Avatar</p>
            </div>
        ...
        <section className="bg-[#111111] border border-white/[0.08] rounded-[2rem] overflow-hidden shadow-2xl relative p-8 md:p-12">
          ...
          <div className="relative">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-8 mb-12 border-b border-white/[0.04] pb-10">
              <div
                className="w-32 h-32 rounded-full overflow-hidden flex items-center justify-center text-4xl font-black shadow-[0_0_30px_rgba(147,51,234,0.3)] border-4 border-[#161616] relative transition-transform"
              >
                {user.avatar ? (
